...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if hasattr(self.user, "_wrapped"):  # Unwrap lazy user
    ...
    def create_message(self, sender, receiver, message):
        cleaned_message = bleach.clean(message, tags=["p", "strong", "em"], strip=True)
        return Message.objects.create(
            sender=sender, receiver=receiver, message=cleaned_message[:1000]
        )
    ...
    def mark_all_as_read(self):
        messages = Message.objects.filter(
            ...
            receiver_id=self.user.id,  # ✅ use .id to avoid LazyObject error
            is_read=False,
        )
