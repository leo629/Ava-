...
class EditProfileForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.form_method = "post"
        self.helper.layout = Layout(
            Row(Column("bio"), Column("gender")),
            Row(Column("country"), Column("profile_pic")),
            Field("interests"),
            Field("hobbies"),
            Field("want_kids"),
...
class GalleryForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "caption": forms.Textarea(
                attrs={"rows": 2, "placeholder": "Add a caption (optional)"}
            )
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Field("image"),
            Field("caption"),
            Submit("submit", "Upload", css_class="btn btn-primary w-full"),
        )
...
class CustomSignupForm(SignupForm):
    ...
    hobbies = forms.CharField(widget=forms.Textarea(
        attrs={"rows": 2}), required=False)
    want_kids = forms.ChoiceField(
        choices=Profile.WANT_KIDS_CHOICES, required=False)
    relationship_goal = forms.ChoiceField(
        choices=Profile.RELATIONSHIP_GOALS, required=False)
    lifestyle = forms.ChoiceField(
        choices=Profile.LIFESTYLE_CHOICES, required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Row(Column("phone_number"), Column("date_of_birth")),
            Row(Column("bio"), Column("gender")),
            Row(Column("country"), Column("profile_pic")),
            Field("interests"),
            Field("hobbies"),
            ...
            Submit(
                "submit",
                "Register",
                css_class="w-full bg-blue-600 text-white hover:bg-blue-700 py-2 rounded mt-4",
            ),
        )
    ...
    def save(self, request):
        ...
        # only if phone_number exists on User
